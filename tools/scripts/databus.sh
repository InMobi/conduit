...
case $startStop in
  ...
  (start)
   ...
   nohup java $DATABUS_OPTS  -Dsun.net.client.defaultConnectTimeout=600000 -Dsun.net.client.defaultReadTimeout=600000 -cp "$CLASSPATH" com.inmobi.databus.Databus $configFile 2>&1 &
   if [ $? -eq 0 ]
    then
